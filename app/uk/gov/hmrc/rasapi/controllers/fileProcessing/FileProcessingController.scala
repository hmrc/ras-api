...
class FileProcessingController @Inject() (
    ...
    (optVersion, withValidJson()) match {
      case (Some(apiVersion), Some(callbackData)) =>
        callbackData.fileStatus match {
          ...
          case STATUS_FAILED =>
            logger.error(
              s"[FileProcessingController][statusCallback] There is a problem with the " +
          ...
          case _             =>
            logger.warn(
              s"[FileProcessingController][statusCallback] There is a problem with the file (${callbackData.reference}) for userId ($userId), the status is:" +
  ...
  private def handleInvalidRequest(
    optVersion: Option[ApiVersion],
    optCallBackData: Option[UpscanCallbackData]
  ): Future[Result] = {
      ...
      case Some(json) =>
        Try(json.validate[UpscanCallbackData]) match {
          ...
          case Failure(exception)             =>
            logger.warn(
              s"[FileProcessingController][withValidJson] Json could not be parsed. Json Data: $json, exception: ${exception.getMessage}"
            )
            None
        }
