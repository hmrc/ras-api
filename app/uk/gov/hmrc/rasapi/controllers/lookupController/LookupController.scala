...
import uk.gov.hmrc.rasapi.controllers.{errorResponseWithErrorsWrites, errorResponseWrites}
  ...
  def getResidencyStatus: Action[AnyContent] = validateAccept(acceptHeaderValidationRules).async { implicit request =>
    ...
    authorised(
      ...
      withValidJson(
        ...
        errors => {
          logger.warn(
            s"[LookupController][getResidencyStatus] The request body could not be parsed for userId ($id): " + errors
              .toString()
          )
          Future.successful(
            BadRequest(errorResponseWithErrorsWrites.writes(ErrorBadRequestResponse(errorConverter.convert(errors))))
          )
        }
      )
    } recoverWith {
