...
class FileController @Inject() (
  ...
  def serveFile(fileName: String): Action[AnyContent] = Action.async { implicit request =>
  ...
  def remove(fileName: String, userId: String): Action[AnyContent] = Action.async { implicit request =>
      ...
      deleteFile(fileName, id)
        .flatMap { res =>
          (parseStringIdToObjectId(fileName) match {
            ...
            case Failure(ex)       =>
              logger.warn(
                s"[FileController][remove] Exception ${ex.getMessage} was thrown," +
  ...
  private def handleAuthFailure: PartialFunction[Throwable, Future[Result]] = {
    case _: InsufficientEnrolments =>
      logger.warn("[FileController][handleAuthFailure] Insufficient privileges")
