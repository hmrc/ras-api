...
class RasFilesSessionService @Inject() (filesSessionRepository: RasFilesSessionRepository)(using
  ...
  def createFileSession(userId: String, reference: String): Future[Boolean] =
    filesSessionRepository
      .put[FileSession](userId)(
        DataKey("fileSession"),
        FileSession(None, None, userId, Some(Instant.now().toEpochMilli), None)
      )
      .map(_ => true)
      .recover { case ex: Throwable =>
        logger.error(
          s"unable to create FileSession to cache => " +
  ...
  def fetchFileSession(userId: String): Future[Option[FileSession]] =
    filesSessionRepository.get[FileSession](userId)(DataKey("fileSession")).recover { case ex: Throwable =>
      logger.error(
        s"unable to fetch FileSession from cache => " +
  ...
  def updateFileSession(
    ...
    (for {
      ...
      logger.error(
        s"[SessionCacheService][updateFileSession] unable to update FileSession => " +
  ...
  def removeFileSession(userId: String): Future[Boolean] =
    filesSessionRepository
      ...
      .recover { case ex: Throwable =>
        logger.error(
          s"[RasFilesSessionService][removeFileSession] unable to remove FileSession from cache  => " +
            s"$userId , Exception is ${ex.getMessage}"
        )
        false
      }
