    ...
    createMatchingData(inputRow) match {
      ...
      case Left(memberDetails) =>
        val result = Await.result(
          desConnector.getResidencyStatus(memberDetails, userId, apiVersion, isBulkRequest = true),
          20.seconds
        )
        ...
        result match {
          ...
          case Right(residencyStatusFailure) =>
            auditResponse(
              failureReason = Some(residencyStatusFailure.code.replace(MATCHING_FAILED, "MATCHING_FAILED")),
              nino = memberDetails.nino,
