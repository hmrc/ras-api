...
trait GridFsTTLIndexing {
  ...
  private def createIndex(mdb: MongoDatabase, collectionName: String, indexName: String): Future[String] = {
    log.info(
      s"[GridFsTTLIndexing][checkAndEnsureTTL] Index does not exist, adding $indexName with TTL option set to $expireAfterSeconds seconds."
    )
    mdb
      .getCollection(collectionName)
      .createIndex(
      ...
      for {
        ...
        ttlResult <- retrieveIndex(mdb, collectionName, LastUpdatedIndex).map { indexes =>
                       indexes.exists(index =>
                         index.containsKey(OptExpireAfterSeconds) && (index.values.toList.contains(
                           BsonInt32(expireAfterSeconds)
                         ) || index.values.toList.contains(BsonInt64(expireAfterSeconds.toLong)))
                       )
                     }
      } yield ttlResult
    }
