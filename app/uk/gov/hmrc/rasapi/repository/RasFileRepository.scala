...
class RasFilesRepository @Inject() (val mongoComponent: MongoComponent, val appContext: AppContext)(using
  ...
  def saveFile(userId: String, reference: String, filePath: Path): Future[ResultsFile] = {
    ...
    val observableToUploadFrom: Observable[ByteBuffer] = Observable(Seq(ByteBuffer.wrap(Files.readAllBytes(filePath))))

    val options: GridFSUploadOptions = new GridFSUploadOptions()
      .metadata(
        Document("reference" -> reference, "fileId" -> reference, "userId" -> userId, "contentType" -> contentType)
      )

    gridFSG
      .uploadFromObservable(reference, observableToUploadFrom, options)
      .head()
      .flatMap { res =>
      ...
      .recover { case e: Throwable =>
        log.error(
          s"[RasFilesRepository][saveFile] Error saving the file $reference for user $userId. Exception: ${e.getMessage}"
        )
    ...
    gridFSG
      ...
      .flatMap {
        case Some(file) =>
          ...
          gridFSG
            .downloadToObservable(file.getObjectId)
            .toFuture()
      ...
      .recover { case ex: Throwable =>
        log.warn(
          s"[RasFilesRepository][fetchFile] Exception while trying to fetch file ${_fileName} for userId ($userId). Exception message: ${ex.getMessage}"
        )
  ...
  def fetchResultsFile(fileId: ObjectId): Future[Option[ResultsFile]] = {
    ...
    gridFSG.find(Document("_id" -> fileId)).headOption().recover { case ex: Throwable =>
      log.error(
        s"[RasFilesRepository][isFileExists] Error trying to find if parent file record exists for id: $fileId. Exception: ${ex.getMessage}"
      )
  ...
  def removeFile(fileName: String, userId: String): Future[Boolean] = {
    ...
    gridFSG.find(Document("filename" -> fileName)).headOption().flatMap {
      case Some(file) =>
        ...
        } yield maybeFile match {
          case Some(file) =>
            log.error(
              s"[RasFilesRepository][removeFile] Error while removing file ${file.getFilename} for userId ($userId)."
            )
            false
          case None       =>
            log.info(
              s"[RasFilesRepository][removeFile] Results file removed successfully for userId ($userId) with the file named $fileName"
            )
            true
        }
      case None       =>
      ...
      log.error(
        s"[RasFilesRepository][removeFile] Error trying to remove file $fileName ${ex.getMessage} for userId ($userId)."
      )
