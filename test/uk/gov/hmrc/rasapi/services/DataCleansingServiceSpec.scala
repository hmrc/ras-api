...
with BeforeAndAfterEach with Logging {
  ...
  "DataCleansingService" should{
    ...
    "not remove chunks that are not orphaned" in {
      await(rasFilesRepository.saveFile("user14","reference14",createFile,"fileId14"))
      await(rasFilesRepository.saveFile("user15","reference15",createFile,"fileId15"))
      val result = await(dataCleansingService.removeOrphanedChunks())
      result.size shouldEqual 0
    }

    "remove orphaned chunks" in  {
      val uploadedFile1: ResultsFile = await(rasFilesRepository.saveFile("user14","reference14",createFile,"fileId14"))
      val uploadedFile2: ResultsFile = await(rasFilesRepository.saveFile("user15","reference15",createFile,"fileId15"))
